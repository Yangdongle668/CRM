...
- **顶栏实时汇率**：USD → CNY、EUR → CNY 实时显示，15 分钟自动刷新（外贸报价常用）
- **Docker 一键部署 / 一键升级**
...
### 13. 汇率小工具（顶栏）
- 所有页面右上角常驻显示 **USD / EUR 对人民币的实时汇率**
- 后端通过 `open.er-api.com`（免费免 key）拉取并内存缓存 15 分钟
- 鼠标悬停显示最近更新时间；每 15 分钟前端自动刷新一次
...
### 汇率
- `GET /rates` - 获取 USD_CNY / EUR_CNY / EUR_USD 实时汇率（服务端缓存 15 分钟）
